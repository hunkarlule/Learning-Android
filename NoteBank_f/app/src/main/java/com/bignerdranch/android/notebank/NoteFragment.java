...
public class NoteFragment extends Fragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (noteId != null) {
            mNote = NoteManager.getNoteManager(getContext()).getNote(noteId);
        }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this.getActivity(), android.R.layout.simple_spinner_item, NoteManager.getNoteManager(getContext()).getCourses(NoteManager.getNoteManager(getContext()).getCourses()));
    ...
    public void onPause() {
        ...
        if (!mNoteDelete) {
            ...
            if (!noteTitle.isEmpty() && !noteText.isEmpty()) {
                ...
                Course course = null;
                for (int i = 0; i < NoteManager.getNoteManager(getContext()).getCourses().size(); i++) {
                    if (mNoteCourse.getSelectedItem().toString().equals(NoteManager.getNoteManager(getContext()).getCourses().get(i).getCourseTitle())) {
                        course = NoteManager.getNoteManager(getContext()).getCourses().get(i);
                    }
                }
                if (noteId == null) {
                    mNote = new Note(course, noteTitle, noteText);
                    NoteManager.getNoteManager(getContext()).addNote(mNote);
                } else {
                    int index = NoteManager.getNoteManager(getContext()).getNotes().indexOf(mNote);
                    NoteManager.getNoteManager(getContext()).getNote(index).setNoteTitle(noteTitle);
                    NoteManager.getNoteManager(getContext()).getNote(index).setNoteText(noteText);
                    NoteManager.getNoteManager(getContext()).getNote(index).setCourse(course);
                }
            }
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.delete_note:
                if (mNote != null) {
                    NoteManager.getNoteManager(getContext()).getNotes().remove(mNote);
                }
                mNoteDelete = true;
                getActivity().finish();
                return true;
